...
def _list_customer_events() -> JSON:
    ...
    return JSON(
        [
            customer_event.to_json()
            for customer_event in get_events_for(
                USER, start=get_datetime("start"), end=get_datetime("end")
            )
        ]
    )
...
def _list_user_events() -> JSON:
    ...
    return JSON(
        [
            user_event.to_json()
            for user_event in list_user_events(
...
def add_user_event() -> JSONMessage:
    ...
    user_event = UserEvent.from_json(request.json, USER.id, only=USER_FIELDS)
    user_event.save()
