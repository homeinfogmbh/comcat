...
def get_damage_reports() -> ModelSelect:
    ...
    return (
        DamageReport.select(UserDamageReport.user, cascade=True)
        .join_from(DamageReport, UserDamageReport)
        .where(UserDamageReport.user == USER.id)
    )
...
def get_user_damage_reports() -> ModelSelect:
    ...
    return UserDamageReport.select(cascade=True).where(UserDamageReport.user == USER.id)
